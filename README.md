...
Pulsemetry React 관리자 콘솔. **2단계: 앱 셸·전역 필터·목업 인증·API 기반과 P2 기본 지표 완료.**
...
- `/`: 개요 준비 화면. `/teams`: 팀 분석 기본 지표. `/operations`, `/scenarios`, `/settings`: 단계별 준비 화면.
...
로그인 후 `/teams`에서 기본 KPI와 차트를 확인합니다. 메뉴의 데이터 표 보기로 수치를 읽을 수 있습니다. `npm run check:teams`는 P2 브라우저 검증이며 실행 서버가 필요합니다. 완료 내용과 다음 단계는 [docs/progress.md](docs/progress.md)를 참고하세요.
