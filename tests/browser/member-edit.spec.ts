...
test("member edits prefill current values, discard cancellation, persist saves and support unassignment", async ({ page }) => {
  ...
  const firstEdit = members.getByRole("button", { name: /구성원 상세$/ }).first();
  const account = (await firstEdit.getAttribute("aria-label"))!.replace(" 구성원 상세", "");
  ...
  await expect(modal.getByRole("button", { name: "취소", exact: true })).toBeDisabled();
  ...
  await expect(save).toBeDisabled();
  await expect(modal.getByRole("button", { name: "취소", exact: true })).toBeDisabled();
  await modal.getByRole("button", { name: "상세 패널 닫기" }).click();
  await expect(modal).not.toBeVisible();
  ...
  const edit = members.getByRole("button", { name: `${account} 구성원 상세`, exact: true });
  await edit.click();
...
test("pending invitations can be edited from either list without changing their expiry", async ({ page }) => {
  ...
  await members.getByRole("button", { name: "edit@example.com 구성원 상세", exact: true }).click();
