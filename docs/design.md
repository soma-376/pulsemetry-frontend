...
- [결정] 기존 AuditDialog(사유 10–500자, 기존 모달 포커스/버튼)를 시나리오 실행과 개인 Widget에 재사용한다. 개요 CSV의 기존 모달에 같은 길이 제약의 사유 textarea를 사용한다.
- [결정] 결과 표의 _compare 필드는 이전, 대응 원본 필드는 현재로 명명한다. 잔존율 코호트 날짜·주차는 기존 표 caption으로 표시한다. 새로운 화면이나 디자인 토큰은 추가하지 않았다. 서버 값/라벨의 실제 시각 검증은 E2E에서 수행한다.

- [검증] 실제 backend 데이터의 주소 도메인 표·현재/이전 비교 표·코호트/주차 caption을 직접 확인했다. 잔존율 1440/768 결과에 페이지 가로 넘침 없음. 새 Figma 시안을 구현한 것이 아니며 기존 모달·카드·표 스타일에 서버 식별 정보를 연결한 결과다.
